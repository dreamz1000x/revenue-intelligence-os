...
export function RecordView({ data }: { data: Record<string, unknown> }) {
  ...
  return <div className="record"><dl>{scalars.map(([name, value]) => <div key={name}><dt>{label(name)}</dt><dd>{name === "status" && typeof value === "string" ? <Status value={value} /> : <Value name={name} value={value} />}</dd></div>)}</dl>{arrays.map(([name, rows]) => <section key={name}><h2>{label(name)}</h2>{rows.length ? <DataTable rows={rows} /> : <Empty>No {label(name).toLowerCase()} recorded.</Empty>}</section>)}</div>;
}
...
export function Metric({ label: text, cents, value }: { label: string; cents?: number; value?: number | string }) { return <article className="metric"><p>{text}</p><strong>{cents === undefined ? value : formatEuro(cents)}</strong></article>; }
export function BackLink() { return <Link className="back" href="/dashboard">← Overview</Link>; }
