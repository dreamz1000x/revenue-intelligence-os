...
describe.sequential("HTTP and PostgreSQL wiring", () => {
  ...
  beforeAll(async () => {
    ...
    const customerPersistence =
      new PostgresCustomerPersistence(database);

    const contractPersistence =
      new PostgresContractPersistence(database);

    const paymentPersistence =
      new PostgresPaymentPersistence(database);

    const refundPersistence =
      new PostgresRefundPersistence(database);
    ...
    app = buildApp({
      ...
      processStripeWebhook: async () => {
        throw new Error(
          "Stripe route must not be called",
        );
      },
  ...
  it(
    "executes the complete Customer to Payment HTTP flow",
    async () => {
      ...
      expect(getCustomer.json()).toEqual(
        createCustomer.json(),
      );
      ...
      expect(getContract.json()).toEqual(
        createContract.json(),
      );
      ...
      const createPayment =
        await app.inject(paymentRequest);
      ...
      expect(getPayment.json()).toEqual(
        createPayment.json(),
      );
      ...
      expect(replayPayment.json()).toEqual(
        createPayment.json(),
      );
        ...
        await database.client.execute<{
          ...
          select
            ...
            (
              ...
              where command_type = 'record_payment'
      ...
      const createRefund =
        await app.inject(refundRequest);
      ...
      const getRefund = await app.inject({
        method: "GET",
        url: "/refunds/1",
      });
      ...
      expect(replayRefund.json()).toEqual(
        createRefund.json(),
      );

      const conflictingRefund =
        await app.inject({
          ...refundRequest,
          payload: {
            ...refundRequest.payload,
      ...
      expect(
        conflictingRefund.statusCode,
      ).toBe(409);

      expect(
        conflictingRefund.json().error.code,
      ).toBe(
        ...
        await database.client.execute<{
          ...
          select
            ...
            (
              select count(*)
              from ledger_entries
              where effect_type = 'refund_recorded'
            ) as ledger_count,
            (
              select count(*)
              from idempotency_records
              where command_type = 'record_refund'
            ) as idempotency_count,
            (
              select string_agg(
                status,
  ...
  it(
    "propagates HTTP idempotency without a duplicate business effect",
    async () => {
      const request = {
        method: "POST" as const,
        url: "/customers",
        headers: {
          "idempotency-key":
            "http-customer-replay",
        },
        payload: {
          displayName: "Replay Customer",
        },
      };
        ...
        await database.client.execute<{
          ...
          select
            ...
            (
              select count(*)
              from idempotency_records
              where command_type = 'create_customer'
            ) as idempotency_count
        `);
