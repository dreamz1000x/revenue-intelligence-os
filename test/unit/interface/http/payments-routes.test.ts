...
function testApp(
  overrides: Partial<
    Pick<HttpUseCases, "recordPayment" | "getPaymentById">
  > = {},
): FastifyInstance {
  const app = buildApp({
...
describe("Payment HTTP interface", () => {
  it(
    "records a Payment with the exact command and explicit ordered serialization",
    async () => {
      let receivedCommand:
        | Parameters<HttpUseCases["recordPayment"]>[0]
        | undefined;
      ...
      expect(
        receivedCommand?.receivedAt.toISOString(),
      ).toBe(RECEIVED_AT.toISOString());
  ...
  it(
    "returns 200 for an idempotent Payment replay",
    async () => {
      const app = testApp({
        recordPayment: async () => ({
          resource: PAYMENT,
      ...
      const response = await app.inject({
        ...
        url: "/payments",
        headers: {
          "idempotency-key": "payment-replay",
        },
        payload: validPaymentPayload(),
      });
  ...
  it(
    "maps malformed JSON to the stable structural error",
    async () => {
      const app = testApp();
  ...
  it("rejects unknown Payment request fields", async () => {
    ...
    const response = await app.inject({
      ...
      url: "/payments",
      headers: {
        "idempotency-key": "unknown-payment-field",
      },
      payload: {
        ...validPaymentPayload(),
        unexpected: true,
  ...
  it.each([
    ...
    "rejects structurally invalid receivedAt %s",
    async (receivedAt) => {
      const app = testApp();
      ...
      const response = await app.inject({
        ...
        url: "/payments",
        headers: {
          "idempotency-key": "invalid-payment-instant",
        },
  ...
  it(
    "requires exactly one Idempotency-Key header",
    async () => {
      const app = testApp();
  ...
  it(
    "maps domain-invalid Payment amounts to 422",
    async () => {
      ...
      const response = await app.inject({
        ...
        url: "/payments",
        headers: {
          "idempotency-key": "invalid-payment-amount",
        },
        payload: {
          ...validPaymentPayload(),
          amountCents: 0,
  ...
  it(
    "maps a missing Contract to CONTRACT_NOT_FOUND",
    async () => {
      const app = testApp({
        recordPayment: async () => {
          throw new ContractNotFoundError(
            createContractId(999),
          );
        },
      });
  ...
  it(
    "maps overpayment to PAYMENT_EXCEEDS_OUTSTANDING",
    async () => {
      const app = testApp({
        recordPayment: async () => {
          throw new PaymentExceedsOutstandingError(
            createMoneyCents(5_001),
            5_000,
          );
        },
      });
      ...
      const response = await app.inject({
        ...
        url: "/payments",
        headers: {
          "idempotency-key": "payment-overpayment",
        },
        payload: validPaymentPayload(),
      });
  ...
  it(
    "maps Payment idempotency conflicts to 409",
    async () => {
      const app = testApp({
        recordPayment: async () => {
      ...
      const response = await app.inject({
        ...
        url: "/payments",
        headers: {
          "idempotency-key": "payment-conflict",
        },
        payload: validPaymentPayload(),
      });
  ...
  it(
    "normalizes equivalent timezone representations to the same instant",
    async () => {
      const receivedInstants: number[] = [];

      const app = testApp({
        recordPayment: async (command) => {
          receivedInstants.push(
            command.receivedAt.getTime(),
          );
      ...
      expect(receivedInstants).toEqual([
        RECEIVED_AT.getTime(),
        RECEIVED_AT.getTime(),
      ]);
    },
  ...
  it(
    "returns and explicitly serializes an existing Payment",
    async () => {
      const app = testApp();
  ...
  it(
    "maps a missing Payment to PAYMENT_NOT_FOUND",
    async () => {
      const app = testApp({
  ...
  it(
    "rejects invalid Payment path syntax without partial parsing",
    async () => {
      const app = testApp();

      const response = await app.inject({
        method: "GET",
        url: "/payments/7suffix",
      });
  ...
  it(
    "maps a syntactically valid but domain-invalid Payment ID to 422",
    async () => {
      const getPaymentById = getPaymentByIdUseCase({
        record: async () => {
          throw new Error(
            "Record Payment must not be called",
          );
        },
        getById: async () => null,
      });
