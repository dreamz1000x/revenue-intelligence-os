...
function testApp(
  overrides: Partial<
    Pick<HttpUseCases, "recordRefund" | "getRefundById">
  > = {},
): FastifyInstance {
  const app = buildApp({
...
describe("Refund HTTP interface", () => {
  it(
    "records a Refund with the exact command and explicit serialization",
    async () => {
      let command:
        | Parameters<HttpUseCases["recordRefund"]>[0]
        | undefined;
      ...
      expect(command?.refundedAt.toISOString()).toBe(
        REFUNDED_AT.toISOString(),
      );
  ...
  it(
    "returns 200 and the stable representation for replay",
    async () => {
      const app = testApp({
        recordRefund: async () => ({
          resource: REFUND,
  ...
  it.each([
    ...
    [
      "refundedAt",
      { refundedAt: "not-an-instant" },
    ],
  ])(
    "rejects malformed %s before application",
    async (_label, override) => {
      ...
      const response = await app.inject({
        ...
        url: "/refunds",
        headers: {
          "idempotency-key": "invalid-refund",
        },
        payload: {
          ...validPayload(),
          ...override,
  ...
  it.each([
    [
      new OriginalPaymentNotFoundError(
        createPaymentId(999),
      ),
      404,
    ...
    [
      new RefundExceedsReversibleAmountError(
        createMoneyCents(31),
        30,
      ),
      422,
    ...
    [
      new IdempotencyPayloadConflict(),
      409,
      "IDEMPOTENCY_PAYLOAD_CONFLICT",
    ],
  ])(
    "maps Refund application errors",
  ...
  it(
    "returns and explicitly serializes an existing Refund",
    async () => {
      const response = await testApp().inject({
        method: "GET",
        url: "/refunds/9",
  ...
  it(
    "maps a missing Refund to REFUND_NOT_FOUND",
    async () => {
  ...
  it(
    "maps caller-originated INVALID_ID validation to INVALID_INPUT",
    async () => {
      const getRefundById = getRefundByIdUseCase({
        record: async () => {
          throw new Error(
            "Record Refund must not be called",
          );
        },
        getById: async () => null,
      });
  ...
  it(
    "sanitizes raw internal INVALID_ID validation as an internal error",
    async () => {
      const internalDetail =
        "persisted Refund ID is invalid";
  ...
  it(
    "sanitizes persisted Refund invariant failures as internal errors",
    async () => {
      const internalDetail =
        "persisted Refund allocations are incoherent";
