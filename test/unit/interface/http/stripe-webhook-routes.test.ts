...
import { TEST_ACCESS_TOKEN_VERIFIER } from "../../../helpers/http-auth.js";
...
function testApp(
  ...
  const dependencies: HttpUseCases = {
    ...
    createContract: async () => {
      throw new Error(
        "Contract route must not be called",
      );
    },
    ...
    recordPayment: async () => {
      throw new Error(
        "Payment route must not be called",
      );
    },
    ...
    recordRefund: async () => {
      throw new Error(
        "Refund route must not be called",
      );
    },
...
describe("Stripe webhook HTTP interface", () => {
  it(
    ...
    async () => {
      ...
      expect(
        ...
        verifiedEvent: expect.objectContaining({
          id: "evt_http123",
        }),
  ...
  it(
    "keeps ordinary Customer JSON parsing unchanged outside the Stripe scope",
    async () => {
      const { app } = testApp();
      ...
      expect(response.json().displayName).toBe(
        "JSON remains parsed",
      );
    },
  );
  ...
  it(
    "enforces the isolated one-MiB Stripe body limit",
    async () => {
  ...
  it(
    "requires exactly one unnormalized Stripe-Signature occurrence",
    async () => {
      const { app } = testApp();
  ...
  it(
    "maps invalid signatures generically without leaking Stripe details",
    async () => {
      const rawPayload = Buffer.from(
        JSON.stringify(stripeEvent()),
        "utf8",
      ...
      const response = await app.inject({
        ...
        headers: {
          "content-type": "application/json",
          "stripe-signature":
            "t=1,v1=secret-cryptographic-detail",
        },
      });
  ...
  it(
    "rejects a one-byte payload change against an otherwise valid signature",
    async () => {
      const rawPayload = Buffer.from(
        JSON.stringify(stripeEvent()),
        "utf8",
  ...
  it.each([
    ...
    "rejects signed unusable JSON or envelopes",
    async (rawPayload, code) => {
      const { app } = testApp();
  ...
  it(
    "rejects live-mode events before persistence",
    async () => {
      const rawPayload = Buffer.from(
        JSON.stringify(
          stripeEvent({
  ...
  it(
    "maps an active claim to the bounded 503 response",
    async () => {
      const rawPayload = Buffer.from(
        JSON.stringify(stripeEvent()),
        "utf8",
  ...
  it(
    "maps conflicting retained evidence to the bounded 409 response",
    async () => {
      const rawPayload = Buffer.from(
        JSON.stringify(stripeEvent()),
        "utf8",
      ...
      const { app } = testApp({
        processStripeWebhook: async () => {
          throw new StripeEventEvidenceConflict(
            "evt_http123",
          );
        },
      });
  ...
  it(
    "logs unexpected failures without payload or signature evidence",
    async () => {
      const payloadMarker =
        "STRIPE_PAYLOAD_MUST_NOT_APPEAR_7F3A";

      const rawPayload = Buffer.from(
        JSON.stringify(
          stripeEvent({
            evidence_marker: payloadMarker,
          }),
        ),
        "utf8",
      );
