...
describe("Customer HTTP interface", () => {
  ...
  it(
    ...
    async () => {
      let receivedCommand:
        | Parameters<HttpUseCases["createCustomer"]>[0]
        | undefined;
      ...
      const response = await app.inject({
        ...
        url: "/customers",
        headers: {
          "IDEMPOTENCY-KEY": "Case-Sensitive_Key!",
        },
        payload: {
          displayName: "Acme",
        },
  ...
  it("returns 200 for an idempotent replay", async () => {
    ...
    const response = await app.inject({
      ...
      url: "/customers",
      headers: {
        "idempotency-key": "customer-replay",
      },
      payload: {
        displayName: "Acme",
      },
  ...
  it(
    "returns an existing Customer and maps a missing one to 404",
    async () => {
      ...
      expect(existing.json().createdAt).toBe(
        FIXED_NOW.toISOString(),
      );

      const missingApp = testApp({
        getCustomerById: async () => null,
      });
  ...
  it.each([
    ...
    "rejects invalid Customer body structure %#",
    async (payload) => {
      const app = testApp();
      ...
      const response = await app.inject({
        ...
        url: "/customers",
        headers: {
          "idempotency-key": "customer-structure",
        },
  ...
  it(
    "maps malformed JSON to the stable 400 response",
    async () => {
      const app = testApp();
  ...
  it(
    "rejects duplicate or ambiguous raw Idempotency-Key occurrences",
    () => {
      expect(() =>
    ...
    async () => {
      const app = testApp({
        getCustomerById: async () => {
          throw new Error(
            "postgres constraint and C:\\private\\path",
          );
        },
      });
