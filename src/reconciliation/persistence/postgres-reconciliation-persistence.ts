...
import { externalSourceEvents, reconciliationActions, reconciliationFindingEvidence, reconciliationFindings, reconciliationRuns } from "./reconciliation-schema.js";
...
export class PostgresReconciliationPersistence implements ReconciliationPersistence {
  ...
  async getFindingById(id: number): Promise<FindingReadModel | null> { const [row] = await this.database.client.select().from(reconciliationFindings).where(eq(reconciliationFindings.id, id)).limit(1); if (!row) return null; const evidence = await this.database.client.select().from(reconciliationFindingEvidence).where(eq(reconciliationFindingEvidence.findingId, id)).orderBy(asc(reconciliationFindingEvidence.id)); const actions = await this.database.client.select().from(reconciliationActions).where(eq(reconciliationActions.findingId, id)).orderBy(asc(reconciliationActions.occurredAt), asc(reconciliationActions.id)); return { finding: fromFinding(row), evidence: evidence.map((item) => { const pairs: Array<[EvidenceEntityType, number | null]> = [["contract", item.contractId], ["installment", item.installmentId], ["payment", item.paymentId], ["refund", item.refundId], ["ledger_entry", item.ledgerEntryId], ["stripe_webhook_event", item.stripeWebhookEventId], ["external_source_event", item.externalSourceEventId]]; const [entityType, entityId] = pairs.find(([, value]) => value !== null)!; return { entityType, entityId: entityId!, role: item.role as never }; }), actions: actions.map((action) => reconstituteReconciliationAction({ ...action, actionType: action.actionType as never, fromStatus: action.fromStatus as never, toStatus: action.toStatus as never })) }; }
